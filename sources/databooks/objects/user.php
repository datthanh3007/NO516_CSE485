...
class User{
    ...
    function bookExist(){
        $query= "SELECT idSach,tenSach,tenTheLoai,idTheLoai FROM " . $this->table_name . "
         WHERE tenSach = ?
                LIMIT 0,1";
         $stmt = $this->conn->prepare( $query );
         $this->email=htmlspecialchars(strip_tags($this->tenSach));
         $stmt->bindParam(1, $this->tenSach);
         $stmt->execute();
         $num = $stmt->rowCount();
         ...
         if($num>0){
            ...
            $this->idsach = $row['idSach'];
            $this->tensach = $row['tenSach'];
            $this->idtheloai = $row['idTheLoai'];
            $this->tentheloai = $row['tenTheLoai'];
           
            return true;  //  because book exists in the database
            }
            return false; // because book does not exist in the database
